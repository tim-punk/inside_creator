...
// 获取脚本
export function GetNodeScript(nd:Node) {
    return nd.getComponent('HexGrid');
}
// 获取地形的地图node
export function GetMapItem(vec: Vec3,list: any[]) {
    for (const itr of list) {
        const item: any = GetNodeScript(itr);
        if ( item && item.hexagon.x == vec.x && item.hexagon.z == vec.z){
            return itr;
        }
    }
    return null;
 ...
 */
export function ComputePath(start: HEXAGON, end: HEXAGON, allPath: any[]):HEXAGON[] {
    const _GetOpenedBest = (openList: AstarNd[])=> { //在打开列表中找到最优节点，并从列表中移除
        let idx = 0;
    ...
    while (openList.length > 0 && !finded) {
       ...
       closeList.push(curr);
       //加入周围可加入的6个坐标值
       const tileList = GetRoundTilePos(curr.item);
       for (let index = 0; index < tileList.length; index++) {
           const ele = tileList[index];
           const nd = GetMapItem(ele,allPath);
           const item: any = GetNodeScript(nd);
           // 可行走，不在关闭列表里面
           if (item && item.canThrough() && !_Contains(item.hexagon,closeList)) {
               const _g = curr.g + 1;
